    ...
    with shelve.open('user.db', 'c') as user_db:
        if str(staff_id).isdigit():
            ...
            for staff in user_db["Staff"].values():
                ...
                elif str(staff.get_staff_id()).isdigit() and int(staff.get_staff_id()) == staff_id:
                    name = staff.get_name()
    ...
    with (shelve.open('user.db', 'c') as user_db):
        ...
        if database_key == "Staff" and cols == 12:
            ...
            for i in range(rows):
                ...
                staff_ids = np.append(staff_ids, user_db["Last ID Used"][2] + i)

            user_db["Last ID Used"] = [user_db["Last ID Used"][0] + rows, user_db["Last ID Used"][1],
            ...
            for a in range(rows):
                ...
                data = Staff(mini_li[0], str(int(mini_li[1])), mini_li[2],
                             mini_li[3], mini_li[4],
            ...
            for i in range(rows):
                user_ids = np.append(user_ids, user_db["Last ID Used"][0] + i+ 1)
                customer_ids = np.append(customer_ids, user_db["Last ID Used"][1] + i)

            user_db["Last ID Used"] = [user_db["Last ID Used"][0] + rows, user_db["Last ID Used"][1] + rows - 1,
            ...
            for a in range(rows):
                ...
                data = Customer(stub[0], str(int(stub[1])), stub[2], stub[3],
        ...
        for i in range(rows):
            var = data_items[i]
            user_id = str(int(user_ids[i]))
            if database_key == "Customer":
                var.set_customer_id(str(int(var.get_customer_id())))
            else:
                var.set_staff_id(str(int(var.get_staff_id())))
...
def home():
    ...
    with shelve.open("user.db", "r") as user_db:
        ...
        area_chart_data = user_db["2023 Earnings"]
...
def create_customer():
    ...
    if request.method == "GET" or (request.method == "POST" and customer_register_form.validate()):
        return render_template('createCustomer.html', form=CustomerForm(), user_id=user_id)
    else:
        with shelve.open('user.db', 'c') as user_db:
            customer_dict = user_db["Customer"]
            if customer_dict:
...
def search_staff():
    ...
    with shelve.open("user.db", "r") as user_db:
        ...
        if user_search_item in staff_dict:
            ...
            return render_template('searchStaff.html', staff=None, user_id=user_id)
    return render_template('searchStaff.html', staff=staff, user_id=user_id)
...
def table():
    ...
    try:
        ...
        return render_template("tables.html", earnings_list=earnings_list, name=name,
                               message=_get_alert_msg(), sh_msg=_get_sh_msg(), user_id=user_id)
...
def register_staff():
    ...
    return render_template('registerStaff.html', form=create_staff_form, message=_get_alert_msg(),
                           sh_msg=_get_sh_msg(), user_id=user_id, name=name)
...
if __name__ == '__main__':
    ...
    try:
        with shelve.open('user.db', 'c') as user_database:
            ...
            # user_database["Staff"] = {}
            # user_database["Last ID Used"] = [0, 0, 0]
            # user_database["Customer"] = {}
